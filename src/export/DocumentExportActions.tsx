...
export function documentViewModel(doc:SalesDocument):InvoiceViewModel{
  const settings=doc.settingsSnapshot;const template=getTemplate(settings?.visual.templateId||'minimal');const seller=settings?.seller;
  return{schemaVersion:1,documentType:doc.documentType,documentNumber:doc.documentNumber||'—',issueDate:doc.issueDate||'—',validUntil:doc.validUntil||undefined,
    seller:{name:seller?.businessName||seller?.displayName||'فروشنده',subtitle:seller?.subtitle||undefined,phone:seller?.phone||undefined,address:seller?.showAddress?seller.address:undefined,telegram:seller?.telegramUsername||undefined,logoUrl:doc.logoPresent?`/api/v1/documents/${encodeURIComponent(doc.id)}/logo`:undefined},
    ...
    payment:doc.documentType==='proforma'?settings?.payment:undefined,signature:{sellerLabel:'مهر و امضای فروشنده',customerLabel:'تأیید خریدار',showStampArea:true},templateId:template.id as TemplateId,templateVersion:template.version,orientation:'landscape',accent:settings?.visual.accent};
}
