...
function Invoices() { return <div className="page"><header className="page-header"><span>آرشیو شما</span><h1>فاکتورها</h1><p>پیش‌نویس‌ها و فاکتورهای ساخته‌شده اینجا نمایش داده می‌شوند.</p></header><EmptyInvoices /></div>; }
...
export function AppShell({ telegram, runtime, data = emptyHome }: { telegram: TelegramAdapter; runtime: TelegramRuntimeSnapshot; data?: HomeData }) {
  ...
  return <main className="app-shell" data-theme={runtime.colorScheme} data-platform={runtime.platform}><div className="app-frame"><div className="topbar"><div className="brand"><span className="brand-mark"><Icon name="spark" /></span><span><strong>میرا</strong><small>فاکتورساز تلگرام</small></span></div><span className="secure-chip"><span/>امن</span></div><div className="content" key={createIntent ? 'create' : tab}>{createIntent ? <div className="page create-placeholder"><span className="create-mark"><Icon name="invoice" /></span><h1>پیش‌فاکتور جدید</h1><p>مسیر ساخت آماده است؛ فرم سریع در گام بعدی به این صفحه متصل می‌شود.</p><button className="secondary-button" onClick={() => setCreateIntent(false)}>بازگشت به خانه</button></div> : tab === 'home' ? <Home data={data} onCreate={startCreate} /> : tab === 'invoices' ? <Invoices /> : <Settings />}</div>
    <nav className="bottom-nav" aria-label="ناوبری اصلی"><button className={tab === 'home' && !createIntent ? 'active' : ''} aria-current={tab === 'home' && !createIntent ? 'page' : undefined} onClick={() => chooseTab('home')}><Icon name="home" /><span>خانه</span></button><button className={tab === 'invoices' && !createIntent ? 'active' : ''} aria-current={tab === 'invoices' && !createIntent ? 'page' : undefined} onClick={() => chooseTab('invoices')}><Icon name="invoice" /><span>فاکتورها</span></button><button className={`nav-create${createIntent ? ' active' : ''}`} aria-label="ساخت پیش‌فاکتور" aria-current={createIntent ? 'page' : undefined} onClick={startCreate}><span><Icon name="plus" /></span><b>پیش‌فاکتور</b></button><button className={tab === 'settings' && !createIntent ? 'active' : ''} aria-current={tab === 'settings' && !createIntent ? 'page' : undefined} onClick={() => chooseTab('settings')}><Icon name="settings" /><span>تنظیمات</span></button></nav></div></main>;
}
...
export default function App() {
  ...
  if (preview) return <AppShell telegram={telegram} runtime={{ ...runtime, colorScheme: new URLSearchParams(window.location.search).get('theme') === 'dark' ? 'dark' : 'light' }} />;
