...
function Home({ data, onCreate, pilotFeedback, onFeedbackSubmitted }: { data: HomeData; onCreate: () => void; pilotFeedback?:PilotFeedbackStatus;onFeedbackSubmitted?:()=>void }) {
...
export function AppShell({ telegram, runtime, data = emptyHome, preview = false, businessId = 'preview', pilotFeedback, onFeedbackSubmitted }: { telegram: TelegramAdapter; runtime: TelegramRuntimeSnapshot; data?: HomeData; preview?: boolean; businessId?: string;pilotFeedback?:PilotFeedbackStatus;onFeedbackSubmitted?:()=>void }) {
  ...
  useEffect(() => { if (preview || createIntent || tab !== 'home') return; let cancelled = false; async function refresh() { try { const current = await listSalesDocuments({limit:3}); const archived = current.documents.length ? null : await listSalesDocuments({archived:true,limit:1}); if (cancelled) return; setHomeData(previous => ({...previous,hasDocuments:current.documents.length > 0 || Boolean(archived?.documents.length),recentInvoices:current.documents.map(doc => ({id:doc.documentNumber || doc.id,customer:doc.customerName,amount:`${BigInt(doc.grandTotalBaseUnit).toLocaleString('fa-IR')} ریال`,date:doc.issueDate || 'پیش‌نویس',status:doc.lifecycleStatus === 'draft' ? 'draft' : 'final'}))})); } catch { if (!cancelled) setHomeData(previous => ({...previous,hasDocuments:true,recentInvoices:[]})); } } void refresh(); return () => { cancelled = true; }; }, [preview, createIntent, tab]);
  ...
  return <main className="app-shell" data-theme={runtime.colorScheme} data-platform={runtime.platform}><div className="app-frame"><div className="topbar"><div className="brand"><span className="brand-mark"><Icon name="spark" /></span><span><strong>فاکتورساز بهار</strong><small>فاکتورساز تلگرام</small></span></div><span className="secure-chip"><span/>امن</span></div><div className="content" key={createIntent ? `create-${documentId??'new'}` : tab}>{createIntent ? createContent : tab === 'home' ? <Home data={homeData} onCreate={startCreate} pilotFeedback={pilotFeedback} onFeedbackSubmitted={onFeedbackSubmitted} /> : tab === 'invoices' ? <DocumentsPage preview={preview} onOpen={openDocument} /> : tab === 'templates' ? <TemplatesPage preview={preview} /> : <Settings onSummaryChange={updateSettingsSummary} />}</div>
