...
final readonly class SalesDocumentService
{
    ...
    /** @return array<string,mixed> */
    private function owned(string $businessId,string $id,bool $lock=false): array { $s=$this->pdo->prepare('SELECT * FROM sales_documents WHERE id=? AND business_id=?'.($lock?' FOR UPDATE':''));$s->execute([$id,$businessId]);$r=$s->fetch();if(!$r)throw new SalesValidationException('document_not_found',404);return $r; }
    /** @return list<array<string,mixed>> */
